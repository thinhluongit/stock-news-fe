...
export const fetchNewsArticle = createAsyncThunk<Article, string, { rejectValue: string }>(
  'news/fetchOne',
...
export const fetchFeaturedNews = createAsyncThunk<Article[], void, { rejectValue: string }>(
  'news/fetchFeatured',
...
const newsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchNews.rejected, (state, action: PayloadAction<string | undefined>) => {
        state.loading = false; state.error = action.payload ?? 'Error';
      })
      .addCase(fetchNewsArticle.pending, (state) => { state.loading = true; state.error = null; })
      .addCase(fetchNewsArticle.fulfilled, (state, action) => { state.loading = false; state.currentArticle = action.payload; })
      .addCase(fetchNewsArticle.rejected, (state, action: PayloadAction<string | undefined>) => {
        state.loading = false; state.error = action.payload ?? 'Error';
      })
      .addCase(fetchFeaturedNews.fulfilled, (state, action) => { state.featuredNews = action.payload; })
