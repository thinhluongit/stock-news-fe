    ...
    <section className="mb-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
        ...
        <Link href={`/news/${main.slug}`} className="lg:col-span-3 group">
          <article className="relative rounded-xl overflow-hidden h-80 bg-gray-800">
            {main.thumbnail_url ? (
              <Image src={main.thumbnail_url} alt={main.title} fill
                className="object-cover group-hover:scale-105 transition-transform duration-300" />
            ) : (
              <div className="w-full h-full bg-gradient-to-br from-green-900/40 to-gray-900 flex items-center justify-center text-6xl">📈</div>
            )}
          ...
          {rest.map((article) => (
            <Link key={article.id} href={`/news/${article.slug}`}
              ...
              <div className="p-3 flex-1 min-w-0">
                ...
                <h4 className="text-sm font-medium text-white group-hover:text-green-400 transition-colors line-clamp-2 mt-0.5">
                  {article.title}
                </h4>
