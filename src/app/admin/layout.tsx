      ...
      <aside className="w-60 flex-shrink-0 bg-gray-900 border-r border-gray-800 flex flex-col">
        ...
        <div className="h-16 flex items-center gap-2 px-4 border-b border-gray-800">
          ...
          <span className="font-bold text-sm text-white leading-tight">
            ...
            <span className="block text-xs text-gray-400 font-normal">{t('admin.panel')}</span>
          </span>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="h-16 bg-gray-900 border-b border-gray-800 flex items-center justify-between px-6 flex-shrink-0">
          <span className="text-sm text-gray-400">
            {user.full_name} <span className="text-gray-600">·</span>{' '}
            <span className="text-xs text-green-400 uppercase">{user.role}</span>
          </span>
          <button
            onClick={() => setLocale(locale === 'en' ? 'vi' : 'en')}
            className="text-xs font-semibold px-2 py-1.5 rounded-lg bg-gray-800 border border-gray-700 text-gray-300 hover:text-white transition-colors"
          >
            {locale === 'en' ? 'VI' : 'EN'}
          </button>
        </header>
