...
export default function AdminStocksPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <h1 className="text-xl font-bold text-white">{t('admin.stocks')}</h1>

      {/* Add form */}
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 space-y-4">
        <h2 className="text-sm font-semibold text-white">{t('admin.stock.add_new')}</h2>
        {addError && (
          <div className="bg-red-500/10 border border-red-500/30 rounded-lg px-3 py-2 text-red-400 text-xs">{addError}</div>
        )}
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
          <div>
            <label className="block text-xs text-gray-400 mb-1.5">{t('admin.stock.symbol')} *</label>
            <input
              type="text"
              value={addSymbol}
              onChange={(e) => setAddSymbol(e.target.value.toUpperCase())}
              placeholder="AAPL"
              className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2.5 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-green-500 font-mono uppercase"
            />
          </div>
          <div>
            <label className="block text-xs text-gray-400 mb-1.5">{t('admin.stock.company')} *</label>
            <input
              type="text"
              value={addCompany}
          ...
          <div>
            <label className="block text-xs text-gray-400 mb-1.5">{t('admin.stock.exchange')}</label>
            <input
              type="text"
              value={addExchange}
          ...
          <div>
            <label className="block text-xs text-gray-400 mb-1.5">{t('admin.stock.sector')}</label>
            <input
              type="text"
              value={addSector}
          ...
          <div>
            <label className="block text-xs text-gray-400 mb-1.5">{t('admin.stock.price')}</label>
            <input
              type="number"
              value={addPrice}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-800">
                  <th className="text-left px-4 py-3 text-gray-400 font-medium">{t('admin.stock.symbol')}</th>
                  <th className="text-left px-4 py-3 text-gray-400 font-medium">{t('admin.stock.company')}</th>
                  <th className="text-left px-4 py-3 text-gray-400 font-medium hidden sm:table-cell">{t('admin.stock.exchange')}</th>
                  <th className="text-left px-4 py-3 text-gray-400 font-medium hidden md:table-cell">{t('admin.stock.sector')}</th>
                  <th className="text-left px-4 py-3 text-gray-400 font-medium">{t('admin.stock.price')}</th>
                  <th className="text-left px-4 py-3 text-gray-400 font-medium hidden lg:table-cell">{t('admin.stock.change')}</th>
                  <th className="px-4 py-3 text-gray-400 font-medium text-right">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-800">
                {stocks.map((s) => (
                  <tr key={s.symbol} className="hover:bg-gray-800/40 transition-colors">
                    <td className="px-4 py-3">
                      <span className="font-mono font-bold text-white">{s.symbol}</span>
                    </td>
                    <td className="px-4 py-3 text-gray-300">{s.company_name}</td>
                    <td className="px-4 py-3 text-gray-400 hidden sm:table-cell">{s.exchange ?? '—'}</td>
                    <td className="px-4 py-3 text-gray-400 hidden md:table-cell">{s.sector ?? '—'}</td>
                    <td className="px-4 py-3">
                      {priceEditSymbol === s.symbol ? (
                        <div className="flex items-center gap-1">
                          <input
                            ...
                            step="0.01"
                            autoFocus
                            className="w-24 bg-gray-800 border border-green-500 rounded px-2 py-1 text-xs text-white focus:outline-none"
                          />
                          <button
